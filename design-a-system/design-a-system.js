...
async function initPartner() {
  ...
  partnerItems.forEach(async (item) => {
    ...
    if (itemPartner === partner?.toLowerCase()) {
      ...
      const introParagraphWrap = document.querySelector(
        "[data-original='intro-wrap']",
      );
      ...
      const designParagraph = item.querySelector(
        "[data-info='design-paragraph']",
      );
      const designParagraphWrap = document.querySelector(
        "[data-original='design-wrap']",
      );
...
const steps = [1, 2, 3].map((n) =>
  document.querySelector(`[data-form-step='${n}']`),
);
...
script.src =
  "https://maps.googleapis.com/maps/api/js?key=AIzaSyDgQwK9IikV6SubEN0XyKTdC3gHB52CEk0&libraries=places&callback=initAutocomplete";
script.async = true;
...
(function () {
  ...
  function smoothScroll(target) {
    ...
    var targetPosition =
      target.getBoundingClientRect().top + startPosition - getOffset();
